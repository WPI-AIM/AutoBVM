...
enum class Motors {  // Class containing all motors that can be switched to (can add more motors if needed in the future)
    STEPPER = 0,
    WIPER
};
...
public:
    ...
    // void change_motor(Motors motor); // Switch between motors
    ...
    // Functions checking when the wiper motor is moving or not.
    void set_wiper_motor_on();
    void set_wiper_motor_off();
    void set_cycle_end_on();
    void set_cycle_end_off();
    ...
    void calculate_trajectory(const float& duration_s, const float& pause_s, const float& goal_pos_deg, float& vel_deg);

    void wiper_set_interval(float interval);

private:
    Stepper stepper;  // Motor object declarations
    Wiper wiper;

    // int motorType = 1;  // 1 = stepper motor, 2 = wiper motor
