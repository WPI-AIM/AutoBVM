...
void Actuator::init()
{
    ...
    // if(motorType == 1)
    // {
    //     stepper.init();  // Initialize stepper motor
    ...
    // if (motorType == 2);
    // {
    //     wiper.init();  // Initialize wiper motor
    // }
}

// void Actuator::change_motor(Motors motor)  // Change motor between wiper and stepper (can add more motors in future)
// {
//     if(motor == Motors::STEPPER)
//     {
//         motorType = 1;
//     }
...
void Actuator::run()
{
    if(ENABLE_WIPER_MOTOR) 
    {
        if(wiper.get_wiperMotorOn() == true) 
        {
            wiper.run();
        }
    }
    ...
    //     stepper.run();  // If stepper motor, run motor
    // }
    // if(motorType == 2)
    // {
    //     if(wiper.get_wiperMotorOn() == true)
    //     {
    //         wiper.run();  // If wiper motor is marked as supposed to be on, run the motor
    //     }      
    // }
    
}

void Actuator::set_wiper_motor_on()  // Set boolean to say that wiper motor is off, set marker for last wiper cycle
{
    wiper.doLastCycle = false;
    wiper.wiperMotorOn = true;
}

void Actuator::set_wiper_motor_off()  // Set boolean to say that wiper motor is on
{
    
    wiper.wiperMotorOn = false;
...
void Actuator::home_expiration()
{
    stepper.home_expiration();
}
...
bool Actuator::is_home()
{
    if(ENABLE_WIPER_MOTOR)
    {
        if(wiper.get_relayOn() == true)
        {
            return false;  // If any relay is on, the wiper motor must not be home
        }
        else return true;  // If both relays are off, wiper is at home position
    }
    else
    {
...
void Actuator::wiper_set_interval(float interval) 
{
    wiper.interval = interval;
}
...
void Actuator::calculate_trajectory(const float& duration_s, const float& pause_s, const float& goal_pos_deg, float& vel_deg)
{
    ...
    // if Wiper, use vel_deg to get the time for each cycle to most closely match the velocity
    // Stepper motor does not change time that each cycle starts, just how fast the pedal moves, wiper does the opposite, bpm still gets faster either way
    // if(ENABLE_WIPER_MOTOR)
    // {
    //     wiper.set_interval((duration_s*1000));   // If using wiper motor, set how long it moves / pauses per cycle
    //     wiper.set_pause(pause_s*1000);
    // }
