...
#define CONFIG_BUTTON_COUNT         10  // Change this if adding buttons to new button total
...
static void add_pressure_select_button()                                        // CURRENTLY UNUSED
{
...
// static void add_stepper_select_button()  // Button creation fucntions to be called at the bottom of this file
// {
//     lv_obj_t* button = add_config_button("Select Stepper Motor");

//     auto event_cb = [](lv_event_t* evt) {

//         auto change_stepper_cb = [](lv_event_t* evt) {   Can change name of event to better reflect function

//             control_change_motor(Motors::STEPPER);         // What is done when the button is pressed
        
//         };
        
//         open_control_confirm_dialog_motor_switch(evt, change_stepper_cb);    Opens warning dialog, use event as input
//     };
    
//     lv_obj_add_event_cb(button, event_cb, LV_EVENT_RELEASED, nullptr);
// }

// static void add_wiper_select_button()
// {
//     lv_obj_t* button = add_config_button("Select Wiper Motor");

//     auto event_cb = [](lv_event_t* evt) {

//         auto change_wiper_cb = [](lv_event_t* evt) {

//             control_change_motor(Motors::WIPER);
        
//         };
        
//         open_control_confirm_dialog_motor_switch(evt, change_wiper_cb);
//     };
    
//     lv_obj_add_event_cb(button, event_cb, LV_EVENT_RELEASED, nullptr);
// }

static void add_volume_mode_select_button()                                    // JOSH PRESSURE
{
    lv_obj_t* button = add_config_button("Select VCV Mode");

    auto event_cb = [](lv_event_t* evt) {

        auto change_volume_cb = [](lv_event_t* evt) {

            control_change_mode(ControlModes::PCV);
        
        };
        
        open_control_confirm_dialog_mode_switch(evt, change_volume_cb);
    };
    
    lv_obj_add_event_cb(button, event_cb, LV_EVENT_RELEASED, nullptr);
}

static void add_pressure_mode_select_button()                                  // JOSH PRESSURE
{
    lv_obj_t* button = add_config_button("Select PCV Mode");

    auto event_cb = [](lv_event_t* evt) {

        auto change_pressure_cb = [](lv_event_t* evt) {

            control_change_mode(ControlModes::PCV);
        
        };
        
        open_control_confirm_dialog_mode_switch(evt, change_pressure_cb);
    };
...
// void open_control_confirm_dialog_motor_switch(lv_event_t* evt, ConfirmChoiceCb confirm_cb, LabelConfigCb label_config_cb)    // Copy this button layout to add buttons
// {
//     lv_obj_t* label_container = open_yes_no_dialog("Confirm Action", true, "Yes", "No", confirm_cb);
//     if (!label_container) {
//         LV_LOG_WARN("Unable to create confirm/deny window. Another window could be open");
...
//         lv_label_set_text(
//                 warning_label_2,
//                 "This operation will only work if the motor is currently OFF.\n"    // Warning window text, edit this to edit that text
//                 "Continue?"
//         );
//     }
...
void open_control_confirm_dialog_mode_switch(lv_event_t* evt, ConfirmChoiceCb confirm_cb, LabelConfigCb label_config_cb)    // JOSH PRESSURE
{
    lv_obj_t* label_container = open_yes_no_dialog("Confirm Action", true, "Yes", "No", confirm_cb);
    if (!label_container) {
        LV_LOG_WARN("Unable to create confirm/deny window. Another window could be open");
    ...
    if (label_config_cb != nullptr) {
        ...
        lv_label_set_text(
                warning_label_2,
                "Make sure ventilator is not running before proceeding.\n"
                "Continue?"
        );
    }
...
void setup_config_window()
{
    ...
    // register_button(add_stepper_select_button);  // Call button creation functions here to actually create the buttons
    // register_button(add_wiper_select_button);
    register_button(add_volume_mode_select_button);                                         // JOSH PRESSURE
    register_button(add_pressure_mode_select_button);
